...
public class WordPlanPresenter extends BasePresenter<IWordPlanView, Activity> {

    private final String TAG = WordPlanPresenter.class.getSimpleName();

    public WordPlanPresenter(IWordPlanView view, Activity activity) {
        super(view, activity);
    }
     ...
     * 单词计划信息
     */
    public void getWordPlan(final Context context) {
                ...
                try {
                    ...
                    if (getView() != null) {
                        ...
                        if (commonBean.getData() != null) {
                            WordPlanBean wordPlanBean = new Gson().fromJson(commonBean.getData().toString(), WordPlanBean.class);
                            getView().showWordPlanResult(wordPlanBean);
                        }
                    }
        ...
        HttpRxObservable.getObservable(ApiUtils.getretrofitApi().getWordPlan(),
     ...
     * 单词计划修改
     */
    public void alterWordPlan(final Context context, final RequestBody body) {
        HttpRxObserver httpRxObserver = new HttpRxObserver(TAG + "getInfo") {
            ...
            protected void onSuccess(Object response) {
                ...
                try {
                    ...
                    if (getView() != null) {
                        getView().closeLoading();
                        getView().showAlterWordPlanResult(commonBean);
                    }
                } catch (JsonSyntaxException e) {
                    e.printStackTrace();
                    ToastUtils.show("解析错误");
        ...
        HttpRxObservable.getObservable(ApiUtils.getretrofitApi().alterWordPlan(body),
                (LifecycleProvider<ActivityEvent>) context, ActivityEvent.PAUSE).subscribe(httpRxObserver);
    }
}
